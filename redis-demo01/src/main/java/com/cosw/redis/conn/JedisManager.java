...
public class JedisManager {
	...
	private void initialPool() {
		 ...
		 * 则得到的jedis实例均是可用的； testOnborrow =true
		 * 很大的消耗性能，从而保证服务器的稳定，可以配合其他配置来避免这一点， 配合testWhileIdle=true（但是默认为false）
		 * 和timeBetweenEvictionRunsMillis来避免这种问题 设置testOnBorrow=false
		 * 产生的问题：http://blog.csdn.net/soberchina/article/details/72953996
		 */
	...
	/**
	 * 初始化切片池 切片池主要用于分布式项目，可以设置主从Redis库
	 */
	...
	private void KeyOperate() {
		...
		// 删除某个key,若key不存在，则忽略该命令;删除成功则返回删除成功个数
